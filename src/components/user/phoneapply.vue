  ...
  <el-main>
    ...
    <div class="table_content">
      <el-table :data="userData.rows" border>
        <el-table-column prop="createdTime" label="申请时间" width="130">
        </el-table-column>
        <el-table-column prop="newPhoneNumber" label="新手机号" >
        </el-table-column>
        <el-table-column prop="oldPhoneNumber" label="老手机号">
        </el-table-column>
        <el-table-column prop="loginDeviceName" label="登录设备" >
        </el-table-column>
        <el-table-column prop="loginRegion" label="登录地区">
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      obtainFeedBackData: async function() {
        ...
        let res = await replacePhoneApply(pamars);
        if (res.code == 0) {
          console.log(res)
          let data = res.data;
          data.rows.forEach((item) => {
            item.createdTime = tooUtils.formatDate(item.createdTime, true);
            item.status = item.status == 0 ? '提交成功' : (item.status == 1 ? '审核中' :(item.status == 2 ?'审核失败':'审核通过'));
          })
          this.userData = data
        }
        // console.log(111, res)
